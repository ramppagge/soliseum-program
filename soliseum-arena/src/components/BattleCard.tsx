...
export function BattleCard({ battle, onClick }: { battle: Battle; onClick?: () => void }) {
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-muted/20 text-xs text-muted-foreground">
            ...
            <span className="font-display font-semibold">{battle.spectators.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-border/50">
          ...
          >
            ...
            <span className="text-sm font-display font-bold text-accent">
              {isLive ? <AnimatedPrize base={battle.prizePool} /> : battle.prizePool.toFixed(1)}
            </span>
            <span className="text-xs text-muted-foreground font-medium">SOL</span>
          </motion.div>
          <span className="text-xs text-muted-foreground font-medium">{battle.startTime}</span>
        </div>
      </div>
    </motion.div>
  );
